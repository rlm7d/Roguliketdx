...
const hexTowerButton = document.querySelector("#hexTowerButton");
...
function updateTowerUI() {
  ...
  hexTowerButton.disabled = !canAffordTower("hex");
}
...
function applyProjectileHit(projectile) {
  ...
  if (projectile.chain) {
    applyChainDamage(projectile, targets);
  }
...
function applyChainDamage(projectile, primaryTargets) {
  const chain = projectile.chain;
  if (!chain) {
    return;
  }
  const alreadyHit = new Set(primaryTargets);
  const candidates = getEnemiesInRadius(projectile.target, chain.range).filter(
    (enemy) => !alreadyHit.has(enemy)
  );
  const chainHits = candidates.slice(0, chain.count);
  const chainDamage = projectile.damage * chain.damageMultiplier;
  chainHits.forEach((enemy) => {
...
function bindControls() {
  ...
  hexTowerButton.addEventListener("click", () => {
    if (currentState !== GameState.BATTLE) {
      return;
    }
    placementMode = "hex";
